...
Processes a requirement through every stage of the pipeline:
...
Updates ``dashboard/state.json`` after each stage so the FastAPI dashboard
...
def _set_stage(stages: list[dict[str, str]], name: str, status: str, detail: str = "") -> None:
    for stage in stages:
        if stage["name"] == name:
            stage["status"] = status
            stage["detail"] = detail
...
def run(requirement_path: str, bench_id: str = "bench_beta") -> None:
    """Execute the full V&V pipeline for a single requirement."""
    stages: list[dict[str, str]] = [
        {"name": "Requirement Ingestion", "status": "pending", "detail": ""},
        {"name": "Quality Assessment", "status": "pending", "detail": ""},
        {"name": "Scenario Decomposition", "status": "pending", "detail": ""},
        {"name": "Abstraction Mapping", "status": "pending", "detail": ""},
        {"name": "Test Generation", "status": "pending", "detail": ""},
        {"name": "Test Execution", "status": "pending", "detail": ""},
        {"name": "Impact Analysis", "status": "pending", "detail": ""},
    ...
    for scenario in scenarios:
        mappings = mapper.map_scenario(scenario, bench_id=bench_id)
    ...
    for artifact in artifacts:
        ...
        icon = "PASS" if result.result == TestResult.PASS else "FAIL"
        print(f"[6/8] Executed {artifact.test_id}: {icon}")
        if result.metrics:
    ...
    _update_state({"stages": stages})
    existing_scenarios_dir = _ROOT / "scenarios" / "existing"
    existing_affected: dict[str, list[str]] = {}
    if existing_scenarios_dir.exists():
        existing_files = list(existing_scenarios_dir.glob("*.feature"))
        if existing_files:
            existing_affected[req.id] = [f.stem for f in existing_files]
    _set_stage(
        stages,
        "Impact Analysis",
        "completed",
        f"{len(existing_affected.get(req.id, []))} existing scenario(s) may be affected",
    )
    _update_state({"stages": stages})
    if existing_affected.get(req.id):
        print(f"[7/8] Impact: {len(existing_affected[req.id])} existing scenarios may overlap:")
        for name in existing_affected[req.id]:
            print(f"       - {name}")
    else:
    ...
    # Build dashboard-friendly traceability records
    trace_rows = []
    for r in records:
        trace_rows.append({
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("path", help="Path to requirement YAML file")
    parser.add_argument(
        "--bench",
        default="bench_beta",
